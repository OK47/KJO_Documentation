...
answers GSEMU's requests:
\begin{sloppypar}
\begin{enumerate}[nosep]
  \item EMU sends \cmd{SET\_FILL\_TARGET} then \cmd{BEGIN\_FILL} to GSEMU.
  \item GSEMU sends \cmd{CAN\_TARE} to LCMU, opens the Fill valve, then
        repeatedly sends \cmd{CAN\_REPORT\_CURRENT\_WEIGHT} to LCMU,
        comparing against its target.
  \item When the target is reached, GSEMU closes the Fill valve itself ---
        no round trip to EMU or LCMU is needed to authorize the close.
\end{enumerate}
\end{sloppypar}
