...
\begin{tikzpicture}[node distance=0.9cm and 1.8cm]
  ...
  \node[hwblock, below=0.7cm of mux, xshift=-1.7cm, minimum width=2.0cm] (nau1) {NAU7802 (1)\\port 3};
  \node[hwblock, below=0.7cm of mux, minimum width=2.0cm] (nau2) {NAU7802 (2)\\port 4};
  \node[hwblock, below=0.7cm of mux, xshift=1.7cm, minimum width=2.0cm] (nau3) {NAU7802 (3)\\port 7};
  % SPI, right column
  ...
  % Each NAU7802 sits on its own independent mux port -- three separate
  % branches, not a chain through each other.
  \foreach \nd in {nau1,nau2,nau3}
    \draw[busline] (mux.south) -- ++(0,-0.35) -| (\nd.north);
...
\sw{KJO\_Thrust\_Buffer} & local  & Pre/post-trigger RAM ring buffer for thrust capture (10~Hz, unified onto the same averaged-read path as weight mode) \\
...
The excitation voltage is set below the NAU7802's rated 2.4--4.5~V range:
...
\begin{itemize}[nosep]
  \item \textbf{I\textsuperscript{2}C failures}: any individual NAU7802 failing to init is shown on the OLED boot status \emph{and} logged to \texttt{Log\_N.txt} (\sw{Cells.begin()} runs after the log file is opened) -- LCMU continues in a degraded state, computing weight/thrust from whichever channels still respond, rather than halting. A DS3231 failure is shown on the OLED boot status \textbf{but not logged to SD} -- the RTC is brought up before the SD card, so \sw{Report\_Status()} calls made during RTC init have no log file open yet to write to.
...
0.4 & September 2026 & Doc-vs-code audit fixes: corrected the Error Handling section (DS3231
                   boot failures are shown on the OLED but not logged to SD -- only NAU7802 and
                   CAN failures are); corrected the NAU7802 LDO voltage range (2.4--4.5~V, not
                   2.4--4.0~V); corrected the \sw{KJO\_Thrust\_Buffer} Module Summary description
                   (10~Hz, not high-rate -- stale since the sample-rate unification); redrew the
                   Hardware Block Diagram's mux/NAU7802 wiring as three independent branches
                   (ports 3/4/7) instead of a misleading daisy chain. \\
\bottomrule
\end{tabularx}
\end{center}
